...
def test_intra_core_tiling_tiles_the_token_axis():
    """The concrete intra-core tiling entry the mapper emits blocks the token axis (not D or N)."""
    gen, workload = _generator(_BIG)
    ...
    node = next(n for n in cns if n.name == node_name)
    tiled_dim = sub.get_dims(node)[int(pos)]
    # the tiled dim is the SEQUENTIAL token axis of the scan
    scan = next(n for n in cns if n.type == "SelectiveScan")
    token_axis = sub.get_dims(scan)[next(iter(sequential_dims(scan)))]
    assert tiled_dim == token_axis
    assert tiling[0]["tile"] < _BIG.seq
