...
def test_an_overlay_namespace_is_picked_up(monkeypatch):
    ...
    monkeypatch.setattr(
        ctx_module,
        "load_group",
        lambda group: [LoadedPlugin("zigzag", AcmeConstraints, "stream-overlay-acme", 20)],
    )
    accelerator = _accelerator(_ZIGZAG)
...
def test_a_broken_strategy_is_skipped_not_raised(monkeypatch):
    ...
    monkeypatch.setattr(
        ctx_module,
        "load_group",
        lambda group: [LoadedPlugin("zigzag", Exploding, "stream-overlay-broken", 20)],
    )
    accelerator = _accelerator(_ZIGZAG)
...
def test_highest_priority_registration_wins(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda group: [
            ...
            LoadedPlugin("zigzag", Override, "stream-overlay-acme", 20),
        ],
    )
    accelerator = _accelerator(_ZIGZAG)
    strategies = namespace_constraints_for(accelerator, _config(accelerator))
