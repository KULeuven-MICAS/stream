...
def test_allowlist_always_includes_the_public_distribution(monkeypatch):
    monkeypatch.setenv(plugins.OVERLAY_ALLOWLIST_ENV, "stream-overlay-acme")
    assert overlay_allowlist() == frozenset({PUBLIC_DISTRIBUTION, "stream-overlay-acme"})
...
def test_load_group_excludes_overlays_outside_the_allowlist(monkeypatch, entry_points):
    ...
    entry_points["stream.decompositions"] = [
        ...
        _EntryPoint("AcmeOp", "acme", "stream-overlay-acme"),
        _EntryPoint("GlobexOp", "globex", "stream-overlay-globex"),
    ]
...
def test_explicit_allow_overrides_the_environment(monkeypatch, entry_points):
    monkeypatch.setenv(plugins.OVERLAY_ALLOWLIST_ENV, "stream-overlay-acme")
    entry_points["stream.decompositions"] = [_EntryPoint("GlobexOp", "globex", "stream-overlay-globex")]
...
def test_higher_priority_overlay_is_registered_last(monkeypatch, entry_points):
    ...
    entry_points[plugins.OVERLAY_GROUP] = [
        _EntryPoint("shared", _overlay(10), "stream-overlay"),
        _EntryPoint("acme", _overlay(20), "stream-overlay-acme"),
    ]
    entry_points["stream.onnx_parsers"] = [
        _EntryPoint("Softmax", "acme", "stream-overlay-acme"),
        _EntryPoint("Softmax", "builtin", PUBLIC_DISTRIBUTION),
        _EntryPoint("Softmax", "shared", "stream-overlay"),
    ]
...
def test_conflicting_registrations_are_logged(monkeypatch, entry_points, caplog):
    ...
    monkeypatch.delenv(plugins.OVERLAY_ALLOWLIST_ENV, raising=False)
    entry_points[plugins.OVERLAY_GROUP] = [_EntryPoint("acme", _overlay(20), "stream-overlay-acme")]
    entry_points["stream.onnx_parsers"] = [
        _EntryPoint("Softmax", "builtin", PUBLIC_DISTRIBUTION),
        _EntryPoint("Softmax", "acme", "stream-overlay-acme"),
    ]
    ...
    assert "stream-overlay-acme" in caplog.text
...
def test_a_broken_plugin_is_skipped_not_raised(monkeypatch, entry_points):
    monkeypatch.delenv(plugins.OVERLAY_ALLOWLIST_ENV, raising=False)
    entry_points["stream.decompositions"] = [
        _EntryPoint("Broken", None, "stream-overlay", broken=True),
        _EntryPoint("Fine", "fine", "stream-overlay"),
    ]
...
def test_a_broken_overlay_declaration_does_not_break_discovery(monkeypatch, entry_points):
    monkeypatch.delenv(plugins.OVERLAY_ALLOWLIST_ENV, raising=False)
    entry_points[plugins.OVERLAY_GROUP] = [_EntryPoint("bad", None, "stream-overlay-bad", broken=True)]
...
def test_loaded_overlays_reports_provenance(monkeypatch, entry_points):
    ...
    entry_points[plugins.OVERLAY_GROUP] = [
        _EntryPoint("acme", _overlay(20), "stream-overlay-acme"),
        _EntryPoint("globex", _overlay(20), "stream-overlay-globex"),
    ]
    assert loaded_overlays() == ("stream-overlay-acme",)
