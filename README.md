...
...or simply `just co-2conv` (this repo uses [`just`](https://github.com/casey/just) as a task runner; defaults to `tpu_like_quad_core` - see the [matrix](#workload-x-hardware-matrix) below). `--mapping` is omitted, so the mapping is auto-generated by the pipeline; the hardware is a TPU-like quad-core system.
...
|---|---|:---:|:---:|
| `eyeriss_like_single_core` | one Eyeriss-like compute core (+ pooling, SIMD, DRAM) | yes | yes |
| `eyeriss_like_dual_core` | two Eyeriss-like compute cores | yes | yes |
| `eyeriss_like_quad_core` | four Eyeriss-like compute cores | yes | yes |
| `tpu_like_quad_core` | four TPU-like compute cores | yes | yes |
| `simba_small` | small Simba chiplet mesh | yes | yes |
| `simba` | 36-core Simba chiplet mesh | yes | yes |
| `fusemax` | FuseMax array + vector + DRAM | yes | yes |
| `meta_prototype_dual_core_simd_offchip` | two Meta-prototype compute cores (+ pooling, SIMD, DRAM) | yes | yes |

yes = completes through the generic CO pipeline. All combinations run in the default fast suite - on these small single-fusion-group workloads even the 36-core `simba` mesh finishes in seconds.
