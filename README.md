...
The small test workloads are produced by builders and are gitignored, so generate them once (this repo uses [`just`](https://github.com/casey/just) as a task runner — see the [matrix](#workload--hardware-matrix) below):

```bash
just gen-workloads
```

Then run the CO pipeline on a small two-Conv workload with an auto-generated mapping (approximately 11 seconds):
...
…or simply `just co-2conv` (defaults to `tpu_like_quad_core`). `--mapping` is omitted, so the mapping is auto-generated by the pipeline; the hardware is a TPU-like quad-core system.
...
**Workloads** (builders in `stream/inputs/testing/workload/`; the generated ONNX are gitignored — run `just gen-workloads` to create them):

- **2-conv** — two chained Conv layers (`make_2_conv.py`).
- **swiglu** — a 4-node SwiGLU block: two Gemms, SiLU, and an elementwise Mul (`make_swiglu.py`).
...
| `simba` | 36-core Simba chiplet mesh | ✓ (slow) | ✓ (slow) |
...
✓ = completes through the generic CO pipeline. The 36-core `simba` cases are marked `slow` (the MILP is the heaviest in the matrix) and are excluded from the default fast suite.
...
```bash
just gen-workloads              # generate the 2-conv + swiglu ONNX (once)
...
**Run the whole matrix** — the `justfile` wraps `pytest tests/test_hardware_combinations.py`, which generates the workloads itself (no `gen-workloads` needed) and also runs a parse-only check confirming all eight hardware definitions load:

```bash
just matrix          # parse + 2-conv + swiglu over all 8 boards (simba excluded — slow)
just matrix-slow     # same, including the slow 36-core simba cases
```
