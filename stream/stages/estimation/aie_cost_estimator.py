...
class AIECostEstimator:
    ...
    def estimate(self, node: ComputationNode, core: Core) -> CoreCostEntry:
        ...
        # A symbol with a measured per-call anchor is costed from silicon; without one, the
        # mapping's hand-fed utilisation percentage; without even a kernel (a generically
        # auto-mapped node), ideal (100%) so the run completes with an ideal-cycle estimate.
