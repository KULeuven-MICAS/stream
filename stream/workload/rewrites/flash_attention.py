...
denominator and output accumulator per query -- O(1) state, independent of the key length. That is a
sequential scan over key blocks, so this rewrite mirrors the M04 chunked pattern (dense per-block
reductions + an inter-block state carry). ``reference.online_softmax_attention`` is the NumPy golden.

This is the public **reference** decomposition (toy: no causal mask, no batching folded in). A private
overlay registers a production flash-attention rewrite (causal/sliding-window masks, tuned block sizes,
calibrated kernels) through the same rewrite-registry interface -- no fork.
"""
...
def build_attention_chain(base: str, seq_q: int, seq_k: int, d_head: int, block: int, dtype) -> Workload:
    ...
    Each block node has iteration dims ``(i, d, c)``: query ``i`` and head ``d`` are PARALLEL, the
    block-local key ``c`` is the REDUCTION (the dense intra-block MACs). It reads the query, its key/
    value block and the incoming running state ``(o, l)``, and writes the outgoing state. Consecutive
    blocks share the state tensors, so the graph carries the inter-block sequential dependency with an
    O(1)-per-query footprint -- the flash property.
    """
    ...
    l_map = AffineMap(3, 0, (dim(0),))  # l[i]     (running softmax denominator)
    ...
    for b in range(n_blocks):
        ...
        nodes.append(
            ComputationNode(
                ...
                inputs=(q, k_block, v_block, o_prev, l_prev),
                # o is the last output so the derived iterator types see (i, d) PARALLEL, c REDUCTION.
                outputs=(l_out, o_out),
                operand_mapping=(q_map, kv_map, kv_map, o_map, l_map, l_map, o_map),
            )
        )
        o_prev, l_prev = o_out, l_out
