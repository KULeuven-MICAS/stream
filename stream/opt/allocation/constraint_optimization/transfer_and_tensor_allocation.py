            ...
            for t in tr.tensors:
                ...
                for c in resources:
                    ...
                    if c.type == "compute":
                        ...
                        for stop in range(-1, len(self.ssis[t].get_applicable_temporal_variables())):
                            # The memory tensor can carry levels its compute tensor does not have,
                            # once the two sides tile differently, and a level that tensor does not
                            # have is not one it can be held across.
                            src_tensor_reuse = (
                                self.z_stop[(compute_tensor, stop)] if stop < compute_levels else 0
                            )
        ...
        for tr in self.transfer_nodes:
            ...
            if tr.transfer_type in (TransferType.COMPUTE_TO_MEM,):
                ...
                for input_tensor in inputs:
                    # Tiling the two sides differently gives them different level counts, so they
                    # pair over the levels both have and the last shared one stands for every
                    # deeper level the longer side has. Each side then still stops exactly once.
                    ...
                    self.model.add_constr(
                        self.model.quicksum(
                            self.z_stop[(output_tensor, s)]._raw for s in range(shared - 1, out_levels)
                        )
                        == self.model.quicksum(
