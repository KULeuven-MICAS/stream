    ...
    def generate_layer_node_for_pooling(self):
        def get_kernel_shape(attrs, ia_dimension_shape):
            ...
            if self.node.op_type in ["MaxPool", "AveragePool"]:
                ...
                kernel_shape = get_attribute_ints_with_name(
                    "kernel_shape", attrs, default=None
                )
            elif self.node.op_type in ["GlobalMaxPool", "GlobalAveragePool"]:
                assert (
                    len(ia_dimension_shape) == 4
                )  # assume the last two dimensions are the pooling kernel dimensions
        ...
        def get_layer_node_input_format(
            kernel_shape, strides, dilations, padding, ia_shape, oa_shape, node_mapping
        ):
            """
            ...
            assert (
                K == C
            ), f"Input and output channels not equal for pooling node {self.node.name}."
            d["loop_dim_size"] = {
                "B": B,
                "K": K,
                "OX": OX,
