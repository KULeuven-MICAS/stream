...
class AcceleratorValidator:
    ...
    SCHEMA = {
        ...
        "2d_mesh": {
            ...
            "required": True,
            "schema": {
                "nb_rows": {"type": "integer", "required": True},
                "nb_cols": {"type": "integer", "required": True},
    ...
    def check_special_core_ids(self) -> None:
        ...
        pooling_core = self.data["2d_mesh"]["pooling_core_id"]
        if pooling_core is not None and pooling_core >= len(self.data["cores"]):
            self.invalidate(
                f"Specified pooling core id {self.data['2d_mesh']['pooling_core_id']} exceeds length of list of cores."
            )
        simd_core = self.data["2d_mesh"]["simd_core_id"]
        if simd_core is not None and simd_core >= len(self.data["cores"]):
            self.invalidate(
                f"Specified simd core id {self.data['2d_mesh']['simd_core_id']} exceeds length of list of cores."
            )
        offchip_core = self.data["2d_mesh"]["offchip_core_id"]
        if offchip_core is not None and offchip_core >= len(self.data["cores"]):
            self.invalidate(
                f"Specified offchip core id {self.data['2d_mesh']['offchip_core_id']} exceeds length of list of cores."
            )

    def check_mesh_layout(self):
        """Check wether the 2d mesh layout works with the given number of cores"""
        nb_special_cores = (
            (1 if self.data["pooling_core_id"] is not None else 0)
            + (1 if self.data["simd_core_id"] is not None else 0)
            + (1 if self.data["offhcip_core_id"] is not None else 0)
        )
        nb_regular_cores = len(self.data["cores"]) - nb_special_cores
        mesh_size = self.data["nb_rows"] * self.data["nb_cols"]
    ...
    def open_core(self, core_file_name: str) -> dict[str, Any] | None:
        ...
        for dir_root_name, dirs_this_dir, files_this_dir in os.walk(input_location):
