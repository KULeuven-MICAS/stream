...
class GenerateCNWorkloadStage(Stage):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.node_output_tensors = (
            {}
        )  # dict that will store the producing cn id for each node's output tensor
        self.output_name_to_node = (
            {}
        )  # dict that will store for each output tensor name which node produced it
    ...
    def run(self):
        ...
        for node in nx.topological_sort(self.workload):
            if not isinstance(
                ...
                (
                    self.node_output_tensors,
                    self.output_name_to_node,
                ) = self.update_node_output_tensors_for_non_cn(
                    node, self.node_output_tensors, self.output_name_to_node
                )
                continue
            ...
            logger.info(
                f"Decoupled {node} to {len(finer_nodes)} smaller CNs with {len(unique_nodes)} unique ones."
            )
            ...
            for input_name, input_operand in zip(
                node.input_names, node.variable_input_operands
            ):  # assume these two are aligned
                try:
                    pred_output_tensor = self.node_output_tensors[input_name]
                except (
                    KeyError
                ):  # This means the input_name is an absolute input of the model
                    continue
                node_input_tensor = tensors_cns[input_operand]
                inter_edges = self.get_inter_edges_tensor_based(
                    pred_output_tensor, node_input_tensor
                )
            ...
            self.node_output_tensors[node_output_tensor_name] = tensors_cns[
                node.output_operand
            ]
        ...
        logger.info(
            f"Finer graph: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges."
        )
        ...
        for node in nx.topological_sort(G):
            ...
            for layer_operand in node.operand_list:
                ...
                if layer_operand in node.constant_operands:
                    tensor.set_base_priority(
                        nb_nodes_per_layer_id[layer_id]
                        - nb_seen_nodes_per_layer_id[layer_id]
                    )
                    ...
                    tensor.set_base_priority(
                        1
                    )  # This currently isn't used (Input tensors are not used)
            nb_seen_nodes_per_layer_id[layer_id] += 1

    @staticmethod
    def update_node_output_tensors_for_non_cn(
        node, node_output_tensors, output_name_to_node
    ):
        ...
        if isinstance(node, ReshapeNode):
            ...
            node_input_names = node.input_names
            assert (
                len(node_input_names) == 2
            ), "ElementwiseNode does not have two inputs."
            node_output_names = node.output_names
            assert (
                len(node_output_names) == 1
            ), "ElementwiseNode does not have one output."
            node_output_name = node_output_names[0]
            # Update the dict by adding an entry for the output based on the joined inputs
            tensors = [
                node_output_tensors[input_name] for input_name in node_input_names
            ]
    ...
    def get_intra_edges(nodes):
        return [
            (nodes[node_id], nodes[node_id + 1], {"size": 0, "bits": 0})
