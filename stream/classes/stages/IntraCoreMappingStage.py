...
class IntraCoreMappingStage(Stage):
    ...
    def __init__(self, list_of_callables, *, workload, accelerator, loma_lpf_limit, **kwargs):
        """
        ...
        for node in self.workload.nodes():
            if not isinstance(node, ComputationNode):
                continue
            equal_nodes = list((unique_node for unique_node in self.unique_nodes if node == unique_node))
            if not equal_nodes:
        ...
        for node in self.unique_nodes:
            if isinstance(node, ComputationNode):
                ...
                raise ValueError(f"IntraCoreMapingStage received node {node} of type {type(node)}.")

        # Initialize dict that will store for all unique nodes their intra-core HW performance for the valid node-core allocations.
        self.node_hw_performances = {unique_node: None for unique_node in self.unique_nodes}  # look-up table

    def run(self):
        logger.info(f'Start IntraCoreMappingStage.')
        # If the kwargs include a node_hw_performances_path, load in the pickle:
        if "node_hw_performances_path" in self.kwargs and path.exists(self.kwargs["node_hw_performances_path"]):
            try:
        ...
        for node in self.unique_nodes:
            ...
            for core_id in self.valid_allocations[node]:
                ...
                # Compute this (node, core) combination's optimal mapping
                    ...
                    node.user_spatial_mapping = core.dataflows  # Set the node's spatial mapping to the possible spatial mappings of the current core
                    # Initialize the flow that will be followed to extract the optimal HW performance of every unique node-core allocation
                    main_stage = self.get_intra_core_mapping_flow(node=node, too_large_operands=too_large_operands_for_cme, core_id=core_id)
    ...
    def check_given_node_hw_performances(self):
        ...
        for unique_node in self.unique_nodes:
            if not unique_node in self.given_node_hw_performances.keys():
                raise ValueError(f"Given node_hw_performances don't match current unique nodes. There is no entry for unique_node {unique_node}.")
    ...
    def check_core_capacity_for_node(self, core: Core, node: ComputationNode):
        ...
        for top_memory in unique_top_memories:
            ...
            layer_operands = [layer_operand_link[mem_operand] for mem_operand in memory_operands]
            ...
            if total_required_capacity <= top_level_capacity:
                ...
                bits_to_be_stored_in_top_level = {k: v for k, v in sorted(bits_to_be_stored_in_top_level.items(), key=lambda item: item[1])}
                nb_operands_in_top_level = len(bits_to_be_stored_in_top_level)
                while top_level_capacity < sum(list(bits_to_be_stored_in_top_level.values())[:nb_operands_in_top_level]):
                ...
                if not operands_stored_in_top_level or not operands_stored_in_offchip:
                    ...
                    rest_capacity = self.get_top_level_memory_rest_capacity(operands_stored_in_top_level, bits_to_be_stored_in_top_level, top_level_capacity)
                    required_capacity = self.get_too_large_operands_minimal_required_capacity_in_top_level_memory(operands_stored_in_offchip, core.dataflows, node)
    ...
    @staticmethod
    def get_top_level_memory_rest_capacity(operands_stored_in_top_level, bits_to_be_stored_in_top_level, top_level_capacity_bits) -> int:
    ...
    def get_too_large_operands_minimal_required_capacity_in_top_level_memory(self, operands_stored_in_offchip, dataflows, node) -> int:
        ...
        mem_op_to_layer_op = dict([(value, key) for key, value in node.memory_operand_links.items()])
    ...
    def add_offchip_to_core(self, core_id, too_large_operands, layer_idx):
        ...
        """
        logger.warning(f"Adding offchip memory for core_id={core_id}, layer={layer_idx}, memory_operands={too_large_operands}.")
        updated_accelerator = pickle_deepcopy(self.accelerator)
        core: Core = updated_accelerator.get_core(core_id)
        offchip_core = pickle_deepcopy(self.accelerator.get_core(self.accelerator.offchip_core_id))
        ## Sanity checks
        ...
        for memory_operand in offchip_memory_operands:
            operand_idx_in_offchip_level = offchip_memory_level.operands.index(memory_operand)
            offchip_port_alloc_raw.append(offchip_memory_level.port_alloc_raw[operand_idx_in_offchip_level])
        ...
        core.memory_hierarchy.add_memory(offchip_memory_instance, offchip_memory_operands, offchip_port_alloc_raw, offchip_served_dimensions)
