...
def fixed_interconnect_with_offchip(
    cores,
    offchip_core,
    bandwidth,
    unit_energy_cost,
):
    """Return a graph of the cores where the cores are connected in sequence.
    Each core get their own dedicated offchip link for parallel weight loading

    Args:
        cores (list): Core objects.
        offchip_core (Core): the offchip core that is added.
        bandwidth (int): The bandwidth in bits of inter-core communication.
    ...
    if offchip_core:
        ...
        assert (
            offchip_read_bandwidth == offchip_write_bandwidth
        ), "Bandwidth behavior undefined for bidirectional links."
        if not isinstance(offchip_core, Core):
            raise ValueError("The given offchip_core is not a Core object.")
        for core in cores:
            # Unique bidirectional communication link per core for parallel weight loading
            link = CommunicationLink(
                offchip_core,
                core,
                offchip_read_bandwidth,
                unit_energy_cost,
                bidirectional=True,
            )
            edges.append((core, offchip_core, {"cl": link}))
            edges.append((offchip_core, core, {"cl": link}))
